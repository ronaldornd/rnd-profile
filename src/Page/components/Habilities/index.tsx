...
export default function Habilities() {
    ...
    return (
        ...
        <Container className="grid xl:h-screen xl:grid-flow-row gap-3 justify-items-center p-3 mb-14 xl:mb-0 w-max">
            ...
            <Row className="grid grid-cols-1 grid-rows-3 gap-3 xl:gap-0 xl:grid-rows-1 grid-flow-col xl:grid-flow-row xl:grid-cols-3 xl:w-max w-full h-full items-center justify-center">
...
<div className="flex flex-col h-full justify-between">
            ...
            <Row className="grid justify-items-center grid-cols-3 grid-rows-2 gap-2 h-max w-full">
                <Row className="col-span-3 flex xl:text-3xl text-2xl w-full h-full justify-center text-center">Projetos</Row>

                <Col className=" col-span-1 xl:row-span-1 row-span-2 border-2 rounded-xl w-max justify-center justify-items-center self-center  border-red-600  " onMouseEnter={() => { setIsOpen(true); handleDialog() }}>
                    ...
                    <Dialog className="fixed flex  w-full h-full p-2 xl:top-1 bottom-0" open={isOpen} onClose={() => { setIsOpen(false) }}>
                        <Dialog.Panel className=" flex 2xl:ml-48 xl:ml-36 xl:h-fit  flex-col w-full h-full rounded-lg border-2 border-red-500 bg-zinc-800">
                            ...
                            <Dialog.Description className="grid gap-2 xl:grid-flow-row py-2 xl:grid-cols-2 ">
                                ...
                                <span className="flex xl:h-[calc(100vh-18rem)] row-span-2 items-center justify-center content-center place-items-center">
                                    ...
                                    <Carousel indicators={false} controls={false} className="xl:h-full  mx-4"
                                ...
                                <span className=" h-10vh mx-4 flex xl:h-32 2xl:h-56 self-end p-3 xl:m-16  text-lg md:text-2xl 2xl:text-5xl rounded-xl border-2 border-red-900 bg-zinc-900 font-thin">{carouselIndex == 0 ? "O objetivo era criar uma aplicação para troca de mensagens entre os usuários em tempo real." : carouselIndex == 1 ? "Nós utilizamos o sistema de autenticação do github para pegar as informações do usuário." : "Os estilos cores e fonte que vocè vê nas imagens eu que escolhi para treinar o que aprendi. "}</span>
                            </Dialog.Description>
                ...
                <Col className=" col-span-1 xl:row-span-1 row-span-2 border-2 rounded-xl h-max w-max justify-center justify-items-center self-center  border-red-600  " onMouseEnter={() => { setIsOpen2(true); handleDialog() }}>
                    <span className="flex p-2 xl:flex-row flex-col items-center w-full justify-center">
                        <img src={logosvg} className="h-10  mx-1" />
                        <h1 className="mx-1 xl:text-sm 2xl:text-xl flex flex-row">
                            Messages
                    ...
                    <Dialog className="fixed flex  w-full h-full p-2 xl:top-1 bottom-0" open={isOpen2} onClose={() => { setIsOpen2(false) }}>
                        <Dialog.Panel className=" flex 2xl:ml-48 xl:ml-36 xl:h-fit  flex-col w-full h-full rounded-lg border-2 border-red-500 bg-zinc-800">
                            <Dialog.Title className="md:text-4xl text-2xl font-bold text-center m-2 w-full">
                            ...
                            <Dialog.Description className="grid gap-2 xl:grid-flow-row py-2 xl:grid-cols-2 ">
                                ...
                                <span className=" h-10vh mx-4  flex xl:h-32 2xl:h-56 self-end p-3 xl:m-16  text-lg md:text-2xl 2xl:text-5xl rounded-xl border-2 border-red-900 bg-zinc-900 font-thin">{carouselIndex == 0 ? "Versão mobile do Live Messages. Procurei fazer o mais parecido o possivel com a versão web." : carouselIndex == 1 ? "Ele foi escrito em React Native utilizando o Expo, tornando-o compativel com android e ios." : "A base de dados é a mesma que a versão web, portanto as mensagens aparecem em todas as versões."}</span>
                            </Dialog.Description>
                        </Dialog.Panel>
                    </Dialog>
                </Col>
                <Col className="row-span-2 xl:row-span-1">
                    <span className="flex gap-2 flex-col h-full items-center xl:flex-row animate-pulse"><span className="row-span-1 flex text-5xl text-center">+</span><span className="row-span-1 flex text-lg xl:text-4xl text-center" >em breve</span></span>
                </Col>
                {/* 
                
                <Col className="col-span-1 xl:row-span-1 row-span-2 p-0 h-full flex flex-col gap-2 xl:flex-row  justify-items-center self-center">


                    <Col className="row-span-1 border-2 rounded-xl w-full border-white xl:justify-center " onMouseEnter={() => { setIsOpen3(true); handleDialog() }} onMouseLeave={() => setIsOpen3(false)}>
                        <span className="flex flex-row items-center xl:justify-center  w-full h-full">
                            <img src={icon} className="2xl:h-10 h-8  mx-1" />
                            <h1 className="xl:text-sm 2xl:text-xl flex flex-row">
                                RND
                                <Planet size={15} className="align-super ml-1" />
                            </h1>

                        </span>
                        <Dialog className="fixed flex xl:w-max xl:h-max h-full p-2 xl:right-9 xl:top-8 bottom-0 xl:mr-8  " open={isOpen3} onClose={() => { setIsOpen3(false) }}>
                            <Dialog.Panel className=" flex p-1 flex-col xl:w-96 w-full h-full justify-between rounded-lg border-2 border-white bg-zinc-800">
                                <Dialog.Title className="xl:text-xl text-2xl font-bold text-center m-2 w-full">
                                    ...
                                    <span className="flex items-center justify-center content-center place-items-center">
                                        ...
                                        >
                                            <Carousel.Item>
                                                <img
                                                    src={live1}
                                                    className="xl:h-full h-96  border-2 rounded-3xl"
                                                />
                    ...
                    <Col className="row-span-1 border-2 rounded-xl w-full border-white  " onMouseEnter={() => { setIsOpen4(true); handleDialog() }} onMouseLeave={() => setIsOpe4(false)}>
                        ...
                        <Dialog className="fixed flex xl:w-max xl:h-max h-full p-2 xl:right-96 xl:top-8 bottom-0 xl:mr-8  " open={isOpen4} onClose={() => { setIsOpen4(false) }}>
                            <Dialog.Panel className=" flex p-1 flex-col xl:w-96 w-full h-full justify-between rounded-lg border-2 border-white bg-zinc-800">
                                <Dialog.Title className="xl:text-xl text-2xl font-bold text-center m-2 w-full">
                                    <button onClick={() => handleDialog()} className="absolute top-5 left-5"><Repeat size={35} /></button>
